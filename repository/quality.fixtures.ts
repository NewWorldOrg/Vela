...
export const QUALITY: QualityResult = {
  ...
  tuners: [
    ...
    },
    {
      ...
      state: {
        ...
        label: '取得できず',
        recap: '再掲 · チューナー',
        sub: '分類 ① 信号を掴めない · 15 時間継続',
      },
      drop: { level: 'nodata', sub: '録画 0 本' },
      lock: { value: '0', unit: '%', sub: '09:41:02 取得 · 信号を掴めず' },
  ...
  anomalies: [
    ...
    },
    {
      ...
      levelLabel: '再掲 · チューナー',
      recap: '再掲 · チューナー',
      body: 'adapter0 · BS / CS · 分類 ① 信号を掴めない · 15 時間継続',
    ...
    },
    {
      ...
      levelLabel: '再掲 · チューナー',
      recap: '再掲 · チューナー',
      body: 'adapter2 · 分類 ④ 内容が食い違う · 設定は地上波 / 検出は衛星',
...
export const MORE_TUNERS_THAN_FIT: QualityResult = {
  ...
  tuners: Array.from({ length: 6 }, (_, round) =>
    QUALITY.tuners.map((tuner) => ({
      ...tuner,
      id: `${tuner.id}-${round}`,
      device: `adapter${round * 4 + Number(tuner.device.slice(7))}`,
    })),
  ).flat(),
}
